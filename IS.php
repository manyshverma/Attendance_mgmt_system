...
<div class="panel-body">
	...
	<a href="view_is.php" class="btn btn-primary">View</a>
	<a href="add_is.php" class="btn btn-primary pull-right">Add</a>	
...
<table class="table">
	...
	if($_SERVER['REQUEST_METHOD']=='POST'){
		...
		$b=true;
		$date=date("Y-m-d");
		$query="select distinct date from iis";
		$result=$link->query($query);$b=false;
			...
			if(!$b){
				foreach($att as $key=> $value){
					if($value=="Present"){
						$query="insert into iis(value,emp_id,date) values('Present','$key','$date')";
						$insertResult=$link->query($query);
						
						
					}
					else{
						$query="insert into iis(value,emp_id,date) values('Absent','$key','$date')";
						$insertResult=$link->query($query);
					}
				}
