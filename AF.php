...
<div class="panel-body">
	...
	<a href="view_af.php" class="btn btn-primary">View</a>
	<a href="add_af.php" class="btn btn-primary pull-right">Add</a>	
...
<table class="table">
	...
	if($_SERVER['REQUEST_METHOD']=='POST'){
		...
		$b=true;
		$date=date("Y-m-d");
		$query="select distinct date from af";
		$result=$link->query($query);$b=false;
			...
			if(!$b){
				foreach($att as $key=> $value){
					if($value=="Present"){
						$query="insert into af(value,emp_id,date) values('Present','$key','$date')";
						$insertResult=$link->query($query);
						
						
					}
					else{
						$query="insert into af(value,emp_id,date) values('Absent','$key','$date')";
						$insertResult=$link->query($query);
					}
				}
